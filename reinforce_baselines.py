...
class WarmupBaseline(Baseline):
    ...
    def eval(self, x, c, cost=None):

        if self.alpha == 1:
            return self.baseline.eval(x, c, cost=cost)
        if self.alpha == 0:
            return self.warmup_baseline.eval(x, c)
        v, l = self.baseline.eval(x, c, cost=cost)
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            ...
            print('baselinedataset self generate:', self.dataset)
        else:
...
class BaselineDataset(Dataset):
    ...
    def __init__(self, dataset=None, baseline=None):
        ...
        self.dataset = dataset
        self.baseline = baseline
        self.cost_data = dataset.cost_data
